...
const TicTacToe = (props) => {
    ...
    //Reading address from the AddressInput component
    const location = useLocation()
    console.log("Logging location state "+location.state)
    const {address1, address2} = location.state
    console.log("Tic component reading values from address component "+address1+" "+address2)

    const [box, setBox] = useState()
    const [boxEth, getBox, saveBox, resultEth, getResult, saveResult,placeBet, transferMoneyToWinner] = useEthConnector(address1,address2)
    ...
    const showResult = (winner) => {
        ...
        saveResult(content);
        transferMoneyToWinner(winner == allConstants.USER_MOVE?'player1':'player2')
    }
a
